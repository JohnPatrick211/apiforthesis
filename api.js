...
app.post('/insertvisitor',(req,res,next)=>{
    let sql = `INSERT INTO acc_user (fname,password,username,phone,address,answer_id) VALUES ('${req.body.name}','${req.body.password}','${req.body.usersid}','${req.body.phone}','${req.body.address}','${req.body.answer_id}')`;
    connection.query(sql, (err,result) =>{
        if(err) throw err;
        console.log(result);
        res.send(result);
...
app.get('/Questioner',(req,res,next)=>{
  let sql = `SELECT * FROM mf_questioner where question_pos = '${req.param("position")}' order by id desc LIMIT 1;`;
  connection.query(sql, (err,result) =>{
      if(err) throw err;
      console.log(res);
...
app.get('/getQuestion',(req,res,next)=>{
  let sql = `SELECT * FROM tbl_question order by position;`;
  connection.query(sql, (err,result) =>{
      if(err) throw err;
      console.log(res);
      res.send(result);
